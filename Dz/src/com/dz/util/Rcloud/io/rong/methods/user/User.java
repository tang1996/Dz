package com.dz.util.Rcloud.io.rong.methods.user;

import com.dz.util.Rcloud.io.rong.RongCloud;
import com.dz.util.Rcloud.io.rong.methods.user.blacklist.Blacklist;
import com.dz.util.Rcloud.io.rong.methods.user.block.Block;
import com.dz.util.Rcloud.io.rong.methods.user.onlinestatus.OnlineStatus;
...
import com.dz.util.Rcloud.io.rong.util.*;
...
public class User {
	 ...
	 * @return TokenResult
	 **/
	public TokenResult join(UserModel user) throws Exception {
		//需要校验的字段
		String message = CommonUtil.checkFiled(user,PATH,CheckMethod.REGISTER);
		if(null != message){
			return (TokenResult)GsonUtil.fromJson(message,TokenResult.class);
	    ...
	    StringBuilder sb = new StringBuilder();
	    sb.append("&userId=").append(URLEncoder.encode(user.id.toString(), UTF8));
	    sb.append("&groupId=").append(URLEncoder.encode("zhuobei", UTF8));
	    sb.append("&groupName=").append(URLEncoder.encode("啄呗红包群", UTF8));
		String body = sb.toString();
	   	if (body.indexOf("&") == 0) {
	   		body = body.substring(1, body.length());
	   	}
	    ...
	    return (TokenResult) GsonUtil.fromJson(CommonUtil.getResponseByCode(PATH,CheckMethod.JOIN,HttpUtil.returnResult(conn)), TokenResult.class);
	}
