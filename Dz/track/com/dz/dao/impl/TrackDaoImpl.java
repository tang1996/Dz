package com.dz.dao.impl;

import java.util.List;

import org.hibernate.Query;
import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

import com.dz.dao.ITrackDao;
import com.dz.entity.Track;

/**
 * 用户DAO实现类
 * 
 * @author GJ
 * @date 2015年4月15日
 */
@Repository("TrackDao")
@SuppressWarnings("unchecked")
public class TrackDaoImpl implements ITrackDao {

	@Autowired
	private SessionFactory sessionFactory;

	public void setSessionFactory(SessionFactory sessionFactory) {
		this.sessionFactory = sessionFactory;
	}

	public SessionFactory getSessionFactory() {
		return sessionFactory;
	}

	// 用户列表
	@Override
	public List<Track> trackList(String sql) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery(sql);
		List<Track> trackList = query.list();
		return trackList;
	}

	public List<Track> getTrack(int orderId) {
		String sql = "SELECT o FROM Track o WHERE o.orderId=:v_orderId ORDER BY o.createTime DESC";
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery(sql);
		query.setInteger("v_orderId", orderId);
		List<Track> trackList = query.list();
		return trackList;
	}
	
	public List<Track> getTracks(int orderId) {
		String sql = "SELECT o FROM Track o WHERE o.orderId=:v_orderId ORDER BY o.id";
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery(sql);
		query.setInteger("v_orderId", orderId);
		List<Track> trackList = query.list();
		return trackList;
	}

	// 删除用户
	@Override
	public void delete(String id) {
		String sql = "DELETE FROM Track o WHERE o.id=:v_id";
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery(sql);
		query.setString("v_id", id);
		query.executeUpdate();
	}

	// 添加或修改信息
	@Override
	public void saveORupdate(Track track) {
		Session session = sessionFactory.getCurrentSession();
		session.saveOrUpdate(track);
	}

}